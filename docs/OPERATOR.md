...
- `PYTH_CORE_API_KEY` must carry a grant for the feeds the markets use. The key supplied on 2026-09-17 authenticates (Hermes answers `Crypto.BTC/USD` and the history API) but its grant excludes the tokenized-stock feeds (`Crypto.NVDAX/USD` and every other `*X`/`*ON` feed, asset type crypto spot) and the US equity reference feeds (`Equity.US.*`): Hermes answers `Not entitled: feed …`. The services then report `blocked: PYTH_CORE_API_KEY grant: …` and, on the fork only, price off the issuers' quotes. Add the xStocks/Ondo crypto feeds and US equities to the plan in the Pyth developer portal; nothing in the code changes.
