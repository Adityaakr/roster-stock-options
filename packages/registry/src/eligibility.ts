...
import { AccountState, ExtensionType, TOKEN_2022_PROGRAM_ID, TOKEN_PROGRAM_ID, getDefaultAccountState, getExtensionTypes, getMint, getPausableConfig, getPermanentDelegate, getScaledUiAmountConfig, getTransferFeeConfig, getTransferHook } from "@solana/spl-token";
...
export interface MintInspection {
  ...
  /** DefaultAccountState: "frozen" means every new token account, a vault included, starts frozen until the issuer thaws it. */
...
export async function inspectMint(connection: Connection, mintAddress: PublicKey): Promise<MintInspection> {
  ...
  if (program === "unknown") {
    return { mint: mintAddress.toBase58(), tokenProgram: "unknown", decimals: 0, supply: "0", extensions: [], freezeAuthority: null, mintAuthority: null, permanentDelegate: null, transferHook: null, transferFee: null, pausable: null, defaultAccountState: null, scaledUiAmount: null, verdict: "ineligible", reason: `owned by ${info.owner.toBase58()}, not a token program` };
  }
  ...
  const das = program === "token-2022" ? getDefaultAccountState(m) : null;
  ...
  const out: MintInspection = {
    ...
    defaultAccountState: das ? (das.state === AccountState.Frozen ? "frozen" : "initialized") : null,
