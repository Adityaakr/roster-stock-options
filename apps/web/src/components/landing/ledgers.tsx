        ...
        <Reveal y={48} style={{ width: "100%", maxWidth: 1110, position: "relative" }}>
          <div className="pf">
            <div className="pf-strip">
              ...
              <div><span className="k">Underwriters live</span><span className="v">{live.length}</span><span className="s">{live.length ? "quoting now" : "none yet on this cluster"}</span></div>
            </div>
            <div className="pf-grid">
              <div className="pf-col">
                ...
                <table className="pf-table">
                  <thead><tr><th>Size</th><th className="num">Per share</th><th className="num">Cost</th><th className="num">Underwriters</th></tr></thead>
                  <tbody>
                      {(call?.ladder ?? []).map((r) => (
                        <tr key={r.size}>
              ...
              <div className="pf-col">
                ...
                <div className="pf-sub">in program-owned vaults, per underwriter</div>
                <table className="pf-table">
                  <thead><tr><th>Underwriter</th><th className="num">USDC</th><th className="num">{sym}</th><th className="num">Account</th></tr></thead>
                  <tbody>
                      {live.length === 0 ? <tr><td colSpan={4} className="muted">No underwriter has quoted yet.</td></tr> : live.map((u) => (
                        <tr key={u.name}>
                          <td>{u.name}</td>
