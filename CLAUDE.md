...
- SPYx mint (from their own docs): `XsEH7wWfJJu2ZT3UCFeVfALnVA6CP5ur7Ee11KmzVpL`. Resolve NVDAx and every other mint from the xStocks Assets API (`https://docs.xstocks.fi/apis/openapi/assets`) or the Tokens API, never from memory.
  - **Correction 2026-09-17 (P0 probe, `fixtures/probes/mint_probe_results.json`):** `XsEH7w…` is **NFLXx** (on-chain TokenMetadata `Netflix xStock`, multiplier 10). SPYx per `api.xstocks.fi/api/v2/public/assets/SPYx` is `XsoCS1TfEyfFhfvj8EtZ528L3CaKBDBRqRapnBbDF2W`; NVDAx is `Xsc9qvGR1efVDFGLrVsmkzv3qi45LTBjeUKSPmx9qEh`. The registry reads mints from the API at every refresh; nothing in code hard-codes them.
