...
| Real marks with no Pyth subscription, and real trade history for every PreStocks token | `packages/services`, `packages/registry/src/geckoterminal.ts` | every market shows which source priced it: Tokens API, Jupiter, the issuer; a PreStocks page charts the mainnet token's closes from its deepest USDC pool and names the pool |
