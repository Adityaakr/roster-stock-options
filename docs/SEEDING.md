...
1. **A fresh program id.** The fork's id `FJUdsdmxAp3zAwZBg3ai34xzeCBDobnH1XDarvVa7uFV` was generated from a keypair that was committed to this repository's history and pushed (feedback finding, `docs/BUILD_LOG.md` M8.1); anyone with the repository could deploy at it first. Before mainnet: `solana-keygen new -o ~/roster-program-keypair.json` outside the repo, put its pubkey in `declare_id!` (`programs/roster_finance/src/lib.rs`), `Anchor.toml` and rebuild (`pnpm anchor:build` re-syncs the IDL and the app reads the id from it). The old keypair is untracked and ignored (`*-keypair.json`) but remains in history until the history is rewritten.
2. `DEPLOY_CLUSTER=<mainnet RPC> DEPLOY_MAINNET_APPROVED=1 pnpm anchor:deploy` with `DEPLOYER_KEYPAIR` (the script refuses anything but the local fork without the flag), then `init_protocol` **in the same shell session immediately after**: `init_protocol` is first-caller-wins, so the window between deploy and init must be seconds.
3. Transfer the upgrade authority to `MULTISIG` immediately: `solana program set-upgrade-authority FJUd… --new-upgrade-authority $MULTISIG --skip-new-upgrade-authority-signer-check` from the deployer. Verify with `solana program show FJUd…` before anything else happens. `docs/UPGRADE.md` is the runbook from then on.
4. `init_protocol` from the deployer with `pause_authority = KEEPER`, `treasury = TREASURY`, `fee_bps = 10`, `integrator_share_bps = 3000`, `keeper_fee_usdc = 2_000_000`, `grace_secs = 3600`. Then `update_protocol` to move `authority` to the multisig (the instruction exists; the SDK builder is `updateProtocol`).
5. `pnpm registry` against the mainnet RPC (read-only), then `pnpm list-markets NVDAx TSLAx SPYx` from the authority: three `create_market` transactions at Tier 1 with the grids below and, per market, the escrow proof (one series, one lot in, one lot out) paid from the deployer's own inventory. Rent per series is in `docs/RENT.md`; the proof's series closes after its expiry.
6. Fund the quoter wallet as below. Start `pnpm services` with `RPC_URL` (Helius), `PYTH_CORE_API_KEY`, `LAUNCH_SYMBOLS=NVDAx,TSLAx,SPYx`. The quoter posts to the two nearest Fridays plus the `EXTRA_EXPIRIES` the operator sets (the brief names a Monday 16:00 NY expiry for the first print).
7. Watch one full expiry: settle and close by the keeper, fee vault reconciled to the sum of `Bought.fee` events, treasury P&L published on the roster.
